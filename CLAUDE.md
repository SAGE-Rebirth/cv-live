...
```bash
# Run the FastAPI server (standard mode with dashboard + terminal controls)
python3 main.py

# Headless mode (terminal-only, no browser needed — for Pi)
...
1. **Main process** (`main.py` → `CameraService` in `src/service.py`)
   ...
   - Runs a health check every 5 seconds that detects crashed child processes and respawns them.
   - Syncs the recorder's FPS with the actual camera FPS reported by the capture process.
...
### Critical: lazy service initialization

**`CameraService` must NOT be constructed at module level in `main.py`.** With `spawn` multiprocessing (default on macOS), child processes re-import `__main__`, which would execute module-level code again. If `CameraService` (and thus `SharedStateManager`) were created at module level, the child process would unlink and recreate the parent's shared memory segment, causing the parent to read all-zero frames and produce blank recordings.

The service is constructed inside the FastAPI `lifespan` context manager and accessed via `get_service()` in route handlers.
...
- `target_fps`, `actual_fps`: the API writes `target_fps`; the capture process writes `actual_fps` after opening the camera. The main loop syncs the recorder to `actual_fps`.
- `target_detection_rate`: the API writes this on `DETECTION_RATE` config changes; the inference process reads it.
...
Two read APIs:
- `get_frame()` — copies out of shared memory. Use this when you need to mutate (overlays, recording, inference).
- `peek_frame()` — zero-copy view onto the live buffer. Use this only for read-only consumers that immediately hand the array to a library that copies internally.

Writes are intentionally lock-free (tearing is acceptable for CV throughput). Shape mismatches are logged as warnings.

### Gesture confirmation logic

Gesture debouncing lives in `GestureDebouncer` (`src/gesture.py`) — a plain class that the camera service feeds with raw `Gesture` enum values from the inference queue. The threshold is `Config.GESTURE_CONFIRMATION_SECONDS` (default 3s, configurable live from the dashboard), and it is the **single source of truth** used by both the action trigger and the dashboard progress overlay. The debouncer fires its `on_start`/`on_stop` callbacks exactly once per hold; releasing the gesture (or losing the hand) resets the state so the same gesture can re-fire.
...
- `src/recorder.py` (`VideoRecorder`): Probes codecs at init (avc1 preferred, mp4v fallback) by writing actual test frames — not just checking `isOpened()`, which lies on macOS. Segments the output every `RECORDING_SEGMENT_DURATION` seconds (wall-clock time, not frame count — robust when FPS changes mid-recording). Enforces `MAX_DISK_USAGE_PERCENT` / `RETENTION_COUNT` (ring-buffer eviction of oldest files) with a max-50-iteration guard to prevent infinite loops. All writer operations are guarded by a `threading.RLock` to prevent races between the API thread (FPS changes) and the main loop thread (write_frame).
...
- `upload_watcher.py` runs as a **separate systemd service** (`cv-uploader.service`). Handles SIGTERM (from systemd stop) by converting it to KeyboardInterrupt for clean shutdown. Scans for finished `.mp4` files, uploads them, quarantines after 3 failures.

### Configuration system (`src/config.py`)

Two pydantic models behind a single `Config` proxy:

- **`_StaticSettings`** (pydantic-settings `BaseSettings`): camera, S3, paths, log level. Loaded from env vars / `.env` at process start, validated by pydantic. Restart required to change.
- **`_RuntimeSettings`** (pydantic `BaseModel`): `DETECTION_RATE`, `MODEL_COMPLEXITY`, `RECORDING_SEGMENT_DURATION`, `MAX_DISK_USAGE_PERCENT`, `RETENTION_COUNT`, `FPS`, `GESTURE_CONFIRMATION_SECONDS`. Loaded from `settings.yaml`, mutated via `Config.update_setting(...)` (called by `POST /api/config`), persisted back to YAML.
...
**Cross-process propagation**: `FPS` and `DETECTION_RATE` are propagated to child processes via shared `multiprocessing.Value`s in `SharedStateManager` (not YAML re-reads). The `apply_runtime_change()` hook in `CameraService` writes to these values after `Config.update_setting()` succeeds. Other runtime settings are read on-demand from `Config` in the main process only.
...
- The CLI supports `--headless`, `--host`, and `--port` arguments. Terminal controls (r/s/q) are available when stdin is a tty.
- The `/api/quit` endpoint triggers a graceful shutdown (SIGINT after a 0.5s delay so the response reaches the client).
